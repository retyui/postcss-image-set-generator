...
export default class GeneratorMiniatures {
  ...
  generateImageSet(result) {
    return `image-set( ${result
      .map(e => e.toString(this.options.get("resolutionType")))
      .join(", ")} )`;
  }
  ...
  valueProcessing(imageSetOption) {
    ...
    return new Promise((resolveImgSet, rejectImgSet) => {
      ...
      this.resolver
        .getPath(originImageUrl) // find img in fs
        .catch(rejectImgSet)
        .then(origPath => {
          if (!origPath) {
            return;
          ...
          scales.filter(e => e <= maxScale).forEach(x => {
            ...
            const newImgFileName = `${_parsed.name}${this.options.get(
              "suffix"
            )}${x}${_parsed.ext}`;
            const newImgFilePath = resolve(_parsed.dir, newImgFileName);

            arrResizePromise.push(
              new Promise((resolveFile, rejectFile) => {
                Newer.skipThisFile(origPath, newImgFilePath).then(
                  skipThisFile => {
                    ...
                    if (skipThisFile) {
                      ...
                      resolveFile(
                        new ImageSetOption()
                          .setDppx(x)
                          .setUrl(originImage.getShortPath(newImgFileName))
                      );
                    } else {
                      originImage
                        .resizeImg(newImgFilePath, x) // created miniature
                        .then(() => {
                          resolveFile(
                            new ImageSetOption()
                              .setDppx(x)
          ...
          return Promise.all(arrResizePromise)
            .then(result => {
