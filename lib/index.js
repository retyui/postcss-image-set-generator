...
import GeneratorMiniaures from './GeneratorMiniaures';
...
export function postcssImageSetGenerator(options) {
	...
	return postcss()
		...
		.use(postcssFunctions({
			functions: {
				'image-set': function resolve(...imageSetOptions) {
					if (imageSetOptions.length === 1 && imageSetOptions[0]) {
						const gm = new GeneratorMiniaures(proxyOptions, resolver);
						return gm.valueProcessing(imageSetOptions[0])
							.catch(e => {
...
export default postcss.plugin('postcss-image-set-generator', postcssImageSetGenerator)
